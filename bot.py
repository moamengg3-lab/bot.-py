...
def apply_cookies(cookies_list_or_dict):
    ...
    if isinstance(cookies_list_or_dict, list):
        ...
        for c in cookies_list_or_dict:
            name   = c['name']
            value  = c['value']
            # استخدام الـ domain الأصلي من الكوكي بدون تغيير
    ...
    # تسجيل دخول فوري بالكوكيز الجديدة في thread منفصل
...
IVASMS_DASHBOARD = {
    ...
    "username": "Boudyshop89@gmail.com",
    "password": "Boudy@015",
...
USERNAME = "Boudyshop89@gmail.com"
PASSWORD = "Boudy@015" 
BOT_TOKEN = "8862801334:AAF_SjduHJXZ9wWK5BUK6oP4RDBGCTPpNUI" 
CHAT_IDS = ["-1004432122733"]
REFRESH_INTERVAL = 3
TIMEOUT = 100
MAX_RETRIES = 5
RETRY_DELAY = 5
...
ADMIN_IDS = [8979149923, 8023409116]
DB_PATH = "bot1.db"
...
def send_welcome(message):
    ...
    # لو أدمن والكوكيز منتهية يبعتله رسالة التجديد
    if is_admin(user_id) and not IVASMS_DASHBOARD.get('is_logged_in', False):
        ...
        bot.send_message(
            chat_id,
            "⚠️ <b>الكوكيز منتهية!</b>\n\n"
            "البوت مش شغال دلوقتي.\n"
            "جدد الكوكيز عشان يكمل 👇",
            reply_markup=mar,
            parse_mode="HTML"
        )
        return
    ...
    if is_maintenance_mode() and not is_admin(user_id):
        maintenance_caption = (
            "<b>❍─── <u>𝙀𝙡𝙕𝙀2𝘽 𝘼𝙇𝙂𝙀𝙉𝙍𝘼𝙇 𝗡𝗨𝗠𝗕𝗘𝗥 𝗢𝗧𝗣</u> ───❍</b>\n\n"
            "<b>⚠️ عذراً عزيزي المستخدم..</b>\n"
            "<b>البوت الآن في وضع الصيانة لتحديث الخدمات.</b>\n\n"
            "<b>⏳ يرجى المحاولة مرة أخرى لاحقاً.</b>\n"
            "<b>────────────────────</b>"
    ...
    fancy_text = (
        ...
        "<b>🎓<u>Support</u> : <a href='tg://user?id=8979149923'>.⤹​الدعم⤾.</a></b>\n\n"
        "<b>────────────────────</b>\n"
...
def handle_country_selection(call):
    ...
    if not force_sub_check(user_id):
        markup = force_sub_markup()
        bot.send_message(chat_id, "<b>🔒خــش الـــقــنـاة عــشــــان تـــعــرف تــســـتـــخــدم الـــبــوت.</b>", parse_mode="TML", reply_markup=markup)
        return
    ...
    # زر الجروب في الأعلى
    markup.add(types.InlineKeyboardButton(" خــش جــــروب الاكـــــواد يــا حـــب🔱", url=""))
...
def change_number(call):
    ...
    markup.add(types.InlineKeyboardButton("اشـتـرك هـنـا يـحـب🔱", url="قناه الاكواد "))
...
def back_to_countries(call):
    ...
    fancy_text = (
        "<b>❍<u>𝙀𝙡𝙕𝙀2𝘽 𝘼𝙇𝙂𝙀𝙉𝙍𝘼𝙇 𝗡𝗨𝗠𝗕𝗘𝗥 𝗢𝗧𝗣</u>❍</b>\n\n"
        "<b>🔋<u>مـنـور بـوت ارقـام يـ صـديـقـي😁✌️</u></b>\n\n"
        "<b>🎓<u>Support</u> : <a href='tg://user?id=tg://user?id=8979149923'>.⤹​الدعم⤾.</a></b>\n\n"
        "<b>────────────────────</b>\n"
        "<b><u>اخـتـر الــدولـة الـتـي تـريـدهـا مـن الـزر الاسـفـل</u> ⬇️</b>"
    )
...
def login_to_ivasms():
    ...
    try:
        ...
        if "login" in dashboard_resp.url.lower():
            print(f"[{dash['name']}] ❌ الكوكيز انتهت — البوت هيوقف المحاولات لحد ما تبعت كوكيز جديدة")
            ...
            # إرسال الإشعار في thread منفصل عشان ميوقفش الـ login
            if not _cookies_alert_sent:
                ...
                _th.Thread(target=_send_alert, daemon=True).start()
            return False
...
def fetch_ivasms_messages():
    ...
    try:
        ...
        # جلب CSRF token من صفحة الـ dashboard
        ...
        if not country_groups:
            print(f"[{dash['name']}] ❌ لا توجد مجموعات!")
            dash['is_logged_in'] = False
            login_to_ivasms()
            return []
    ...
    try:
        ...
        keyboard = {
            "inline_keyboard": [
                ...
                [
                    {"text": "💬 𝑆𝑀𝑆 ↗️", "url": "جروب الاكواد بتاعك"},
                    {"text": "𝐵𝑂𝑇 ↗️", "url": "https://t.me/البوت هنا"}
                ],
                
                # الصف الثالث: زر المطور
                [
                    {"text": "𓆩𝘽𝙤𝙩 𝘾𝙝𝙖𝙣𝙣𝙚𝙡𓆪 ↗️", "url": "https://t.me/قناه البوت"} 
                ]
            ]
        }
    except Exception as e:
    ...
    for chat_id in CHAT_IDS:
        try:
            ...
            if resp.status_code == 200:
                ...
                # delete message after 180 sec
                msg_id = resp.json()["result"]["message_id"]
                threading.Thread(
                    target=delete_message_after_delay, 
                    args=(chat_id, msg_id, 180), 
                    daemon=True
                ).start()
            else:
...
def receive_new_cookies(message):
    ...
    try:
        ...
        # apply_cookies بتشغّل login تلقائياً - بس نبعت رسالة تأكيد بعد شوية
        def check_login():
            import time as _t
            _t.sleep(5)  # استنى 5 ثواني عشان login يخلص
        ...
        bot.reply_to(message, "❌ خطأ: " + str(e))
...
_INITIAL_COOKIES = [
    {
        "name": "XSRF-TOKEN",
        "value": "%3D",
...
apply_cookies(_INITIAL_COOKIES)
